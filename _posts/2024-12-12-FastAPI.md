...
• Web API: an API that uses Hypertext Transfer Protocol (HTTP) to transport data
...
Next, we need to feed the decorator the function we want to modify. We'll do that by defining a function called read_root that will retrieve the contents of the page at "/" for us. In this case the content of the page will be a message we create in a python dictionary. FastAPI will translate our python dictionary into JSON, which is the format web app requests and responses are typically sent in. This feature speeds up development because it allows us to do all of our work in python.
...
Next, we'll need to define a class for our items.
...
Next, we are defining the characteristics of the items in our inventory. They will need a name of type string, a price of type float, and may include an optional description.
...
Great! Now we want to create an inventory to store our items in. For this demo, we'll be using an in-memory data store. However, in production you would want to use some kind of database. We can create our in-memory data store by defining an empty dictionary called inventory.
...
Next, we are feeding the decorator a function called get_item and defining the {item_id} path parameter as a parameter as specifying it must be an integer. After that, we are defining an optional query parameter called "name" that needs to either be a string, or bypassed and set to None.

```
def get_item(item_id: int, name: Optional[str] = None):
```

After that, we are using an if statement to check if that item_id we passed is in the inventory. If not, it will raise an exception that returns a 404 status code and the following message: "Item ID not found."
...
Once again, we are creating a decorator for our app. THis time, however, we are calling the POSt method after the dot. After that, we give it a descriptive path name and pass item_id as a path parameter once again.
...
Let's cover some background information we'll need in order to run our API.
